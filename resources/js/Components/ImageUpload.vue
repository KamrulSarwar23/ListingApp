...
<template>
    <div>
        ...
        <label for="image" class="block rounded-md mt-1 bg-slate-300 h-[140px] overflow-hidden  cursor-pointer border-slate-300 border relative" :class="{'!border-red-500' : oversizedImage}">
            ...
            <button @click.prevent="revertImageSelect" class="absolute top-2 right-2 bg-white/75 w-8 h-8 rounded-full grid place-items-center text-state-700" v-if="showRevertBtn" type="button">
